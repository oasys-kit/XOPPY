...
try:
    from orangecontrib.xoppy.util.xoppy_calc import xoppy_calc_xshwig
except ImportError:
    print("compute pressed.")
    print("Error importing: xoppy_calc_xshwig")
    raise
...
class OWxshwig(widget.OWWidget):
    ...
    def unitLabels(self):
         return ['Electron Energy [GeV]','User unit (length)','Use emittances','Sigma X [cm]','Sigma Z [cm]','Emittance X [rad.cm]','Emittance Z [rad.cm]','Distance from X waist [cm]','Distance from Z waist [cm]', 'Type','Number of periods','ID period [m]','K value','file with B vs Y','file with harmonics', 'Number of rays','Seed (0=clock)','Photon energy from [eV]','Photon energy to [eV]','Optimize source (reject rays)','File with phase space volume','Max number of rejected rays (set 0 for infinity)','Slit distance [cm] (set 0 for angular acceptance)','Min X [cm] (or min Xp if distance=0)','Max X [cm] (or max Xp if distance=0)','Min Z [cm] (or min Zp if distance=0)','Max Z [cm] (or max Zp if distance=0)']
